...
int main(int argc, char **argv) {
  ...
  */
  ...
  std::cout << "Use the following keys to control the cropping region"
            << std::endl;
  std::cout << "  Q/W -> Increment/Decrement the topleft X position"
            << std::endl;
  std::cout << "  A/S -> Increment/Decrement the topleft Y position"
            << std::endl;
  std::cout << "  E/R -> Increment/Decrement the bottomright X position"
            << std::endl;
  std::cout << "  D/F -> Increment/Decrement the bottomright Y position"
            << std::endl;
  ...
  while (rclcpp::ok()) {
    ...
    if (sendSignal) {
      std::cout << "Top left Position -> (" << request->top_left.x << ", "
                << request->top_left.y << ")" << std::endl;
      std::cout << "Bottion right Position -> (" << request->bottom_right.x
                << ", " << request->bottom_right.y << ")" << std::endl;
      ...
      if (rclcpp::spin_until_future_complete(node, result) ==
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),
                     "Failed to call service crop_control_srv");
      }
      sendSignal = false;
    }
  }
